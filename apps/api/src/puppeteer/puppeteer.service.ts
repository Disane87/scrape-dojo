...
export class PuppeteerService implements OnModuleDestroy {
  ...
  private getExecutablePath(): string {
    ...
    if (!chromePath) {
      this.logger.warn(
        `⚠️ No Chrome/Chromium found, using Puppeteer's bundled Chromium`,
      );
  ...
  public async newPage(): Promise<puppeteer.Page> {
    ...
    if (pages.length > 1) {
      this.logger.log(
        `🧹 Closing ${pages.length - 1} extra page(s) from session restore`,
      );
    ...
    if (this.currentPage.isClosed()) {
      this.logger.warn(
        '⚠️ Page was closed before configuration, creating new one',
      );
      this.currentPage = await browser.newPage();
    }
    ...
    try {
      ...
      if (
        ...
        this.logger.warn(
          '⚠️ Page closed during configuration, creating new page',
        );
        this.currentPage = await browser.newPage();
        await this.currentPage.setViewport({
          width: 1920,

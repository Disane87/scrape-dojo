...
export class DatabaseService implements OnModuleInit {
  ...
  async updateActionStatus(
    ...
    return this.actionRepo.updateStatus(
      actionId,
      status,
      error,
      result,
  ...
  async deleteRunsByScrapeId(scrapeId: string): Promise<number> {
    const result = await this.dataSource
      ...
      .delete({ scrapeId });
    this.logger.log(
      `🗑️ Deleted ${result.affected} runs for scrape: ${scrapeId}`,
    );
  ...
  async storeData(
    ...
    if (existing) {
      ...
      this.logger.debug(
        `💾 Updated scrape data: ${scrapeId}.${key} = ${value.substring(0, 50)}...`,
      );
      return saved;
    } else {
      // Neuer Eintrag
      ...
      this.logger.debug(
        `💾 Stored new scrape data: ${scrapeId}.${key} = ${value.substring(0, 50)}...`,
      );
      return saved;
    }
  }
  ...
  async getAllScrapeData(
    scrapeId: string,
  ): Promise<{ jobLevel: ScrapeData[]; runs: Record<string, ScrapeData[]> }> {
    const scrapeDataRepo = this.dataSource.getRepository(ScrapeData);
  ...
  async getOrCreateSchedule(scrapeId: string): Promise<ScrapeSchedule> {
    ...
    if (!schedule) {
      this.logger.debug(
        `📅 No schedule found, creating default for: ${scrapeId}`,
      );
      ...
      this.logger.debug(
        `📅 Found schedule for ${scrapeId}: scheduleEnabled=${schedule.scheduleEnabled}, cron=${schedule.cronExpression}`,
      );
    }
  ...
  async updateScheduleConfig(
    ...
    const schedule = await this.getOrCreateSchedule(scrapeId);
  ...
  async updateSecret(
    id: string,
    updates: { name?: string; encryptedValue?: string; description?: string },
      ...
      return childActions.map((childAction: any) => {
        const mapped: any = {
          ...
          status: childAction.status
            ? mapStatus(childAction.status)
            : childAction.status,
          // Recursively process nested child actions
          childActions: childAction.childActions
            ? mapChildActionsStatus(childAction.childActions)
            : childAction.childActions,
        };

        // Process nested loop iterations (for loops inside loops)
        if (
          childAction.loopIterations &&
          Array.isArray(childAction.loopIterations)
        ) {
          mapped.loopIterations = childAction.loopIterations.map(
            (iteration: any) => ({
              ...iteration,
        ...
        actions: (step.actions || []).map((action: any) => {
          ...
          if (action.loopData) {
            try {
              ...
              this.logger.warn(
                `Failed to parse loop data for action ${action.actionName}: ${error.message}`,
              );
            }
          } else if (action.actionType === 'loop') {
            this.logger.warn(
              `⚠️ Loop action ${action.actionName} has no loopData!`,
            );
          }

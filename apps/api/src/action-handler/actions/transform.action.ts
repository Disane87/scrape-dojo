...
export class TransformAction extends BaseAction<TransformActionParams> {
  async run(): Promise<unknown> {
    ...
    if (this.params.previousDataKey) {
      ...
      this.logger.debug(
        `Using previousDataKey "${this.params.previousDataKey}" as root: ${JSON.stringify(inputData)}`,
      );
      ...
      inputData = dataObj;
      this.logger.debug(
        `Using previousDataKeys as root: ${JSON.stringify(inputData)}`,
      );
    } else if (this.params.data) {
      // Wenn data angegeben, wird das Objekt als Root verwendet
      inputData = this.params.data;
      this.logger.debug(
        `Using data object as root: ${JSON.stringify(inputData)}`,
      );
    } else {
      ...
      this.logger.debug(
        `Using all previousData as root (${this.previousData.size} entries)`,
      );
    }
    ...
    if (
      ...
      if (
        ...
        this.logger.debug(
          `Added currentData to context: ${JSON.stringify(currentData)}`,
        );
      }
    }

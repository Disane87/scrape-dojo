...
export class TypeAction extends BaseAction<TypeActionParams> {
    private async findFirstUsableElement(selector: string): Promise<ElementHandle<Element>> {
    ...
    async run(): Promise<void> {
        ...
        if (!typedValue || typedValue.length === 0) {
            this.logger.debug(`⚠️ Value not present after typing; falling back to direct value set + input events`);

...
export class LoopAction extends BaseAction<LoopActionParams> {
    ...
    async run(): Promise<any> {
        ...
        const iterations: Array<{
            ...
            value: unknown;
            status: string;
            childActions: Array<{ name: string; actionType?: string; status: string; result?: any; error?: string }>;
        }> = [];
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            const iterationData = {
                ...
                value: element,
                status: 'running',
                childActions: [] as Array<{ name: string; actionType?: string; status: string; result?: any; error?: string }>
            };

            // Aktualisiere den loopPath für diese Iteration
            const currentLoopPath = [...parentLoopPath, { name: this.name, index: i }];
            this.data.loopPath = currentLoopPath;

            // Emit Loop-Iteration Event
            if (scrapeEventsService && scrapeId) {
                const loopValue = typeof element === 'string' || typeof element === 'number'
                    ? String(element)
                    : `Element ${i + 1}`;
            ...
            this.logger.log(`🔄 Loop "${this.name}": Iteration ${i + 1}/${elements.length}`);
            ...
            for (const actionConfig of this.params.actions) {
                ...
                try {
                    ...
                    if (this.data.skipCurrentIteration) {
                        this.logger.debug(`⏭️ Skipping remaining actions in iteration ${i + 1}`);
                    ...
                    if (errorMessage === 'BreakLoop') {
                        ...
                        if (breakLevels > 1) {
                            ...
                            this.logger.warn(`🛑 Breaking loop "${this.name}" (${breakLevels - 1} more levels to break)`);
                    ...
                    this.logger.error(`Error executing action "${actionConfig.action}": ${errorMessage}`);
        ...
        this.logger.log(`✅ Loop "${this.name}" completed all ${elements.length} iterations`);

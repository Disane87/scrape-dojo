...
export class DownloadAction extends BaseAction<DownloadActionParams> {
  async run(): Promise<string> {
    ...
    try {
      ...
      if (!filename || filename.trim() === '') {
        this.logger.error(
          `❌ Filename is empty! Check template variable resolution.`,
        );
        this.logger.debug(
          `Previous data: ${JSON.stringify(Object.fromEntries(this.previousData))}`,
        );
      ...
      if (scrapeId) {
        ...
        if (segments.length > 0) {
          ...
          this.logger.debug(
            `📂 Reorganized path with scrape subfolder: ${targetPath}`,
          );
        } else {
      ...
      return await this.downloadRemoteFile(
        resolvedUrl,
        fullPath,
        cleanFilename,
      );
      ...
      this.logger.error(
        `❌ Error while downloading file from URL: ${url}. Error: ${error.message}`,
      );
      return null;
    }
  }
  ...
  private async downloadLocalFile(
    ...
    const normalizedPath =
      process.platform === 'win32' && sourcePath.startsWith('/')
    ...
    this.logger.log(
      `✅ Copied local file: ${path.basename(fullPath)} (${fileSizeKB} KB)`,
    );

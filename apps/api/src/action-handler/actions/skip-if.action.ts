...
export class SkipIfAction extends BaseAction<SkipIfParams> {
    async run(): Promise<boolean> {
        const { condition, breakLoop = false, breakLevels = 1, message } = this.params;
        ...
        if (shouldAct) {
            if (breakLoop) {
                const logMessage = message || `🛑 Breaking loop: condition "${condition}" = ${conditionValue}`;
                ...
                const logMessage = message || `⏭️ Skipping: condition "${condition}" = ${conditionValue}`;
                this.logger.log(logMessage);
            ...
            this.logger.debug(`➡️ Continuing: condition "${condition}" = ${conditionValue}`);
        }

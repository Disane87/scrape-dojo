  ...
  async run(): Promise<boolean> {
    ...
    try {
      ...
      if (this.params.element) {
        elementHandle = getValueFromPath<ElementHandle<Element>>(
          this.data,
          this.params.element,
        );
      }
      ...
      if (elementHandle) {
        if (this.params.selector) {
          const selectorElementHandle = await elementHandle.waitForSelector(
        ...
        const selectorElementHandle = await this.page.waitForSelector(
          this.params.selector,
          { timeout },
        );
        if (!selectorElementHandle) {

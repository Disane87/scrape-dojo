...
export class WaitForSelectorAction extends BaseAction<WaitActionParams> {
    async run(): Promise<boolean> {
        ...
        if (this.params.condition !== undefined) {
            ...
            if (!conditionMet) {
                this.logger.log(`⏭️ Skipping wait: condition not met (${this.params.condition})`);
                return false;
            }
        ...
        this.logger.log(`⏳ Waiting for: ${this.params.selector} (timeout: ${timeout}ms, optional: ${optional})`);

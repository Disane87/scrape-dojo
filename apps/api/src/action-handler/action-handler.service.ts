...
export class ActionHandlerService implements OnModuleInit {
    ...
    onModuleInit() {
        ...
        this.actions.forEach((action: RegisteredAction) => {
            logger.debug(`✅ Action registered: ${action.name} (${action.metadata.displayName})`);
        });
        
        logger.debug(`📋 Total actions registered: ${this.actions.length}`);
        logger.debug(`📝 Registered action names: ${this.actions.map(a => a.name).join(', ')}`);
    }

    async handleAction(scrapeAction: ScrapeAction<unknown>, previousData: PreviousData, data?: ScrapeActionData, storedData?: object, runVariables?: Record<string, any>): Promise<any> {
        ...
        const variablesMap = await this.variableStrategy.resolve(data?.scrapeId, effectiveRunVariables);
        
        if (effectiveRunVariables && Object.keys(effectiveRunVariables).length > 0) {
            this.logger.log(`✨ Using ${Object.keys(effectiveRunVariables).length} run-time variables (overriding DB values)`);
        }

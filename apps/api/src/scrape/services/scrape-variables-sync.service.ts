...
export class ScrapeVariablesSyncService {
        ...
        for (const scrape of uniqueWorkflows.values()) {
            ...
            for (const varDef of scrape.metadata.variables) {
                try {
                    ...
                    const { isSecret, secretId } = await this.resolveSecretRef(varDef.secretRef, scrape.id, varDef.name);
                    ...
                    syncedCount++;
                    this.logger.debug(`🔄 Synced variable ${varDef.name} for workflow ${scrape.id}`);
                } catch (error) {
                    this.logger.warn(`⚠️ Failed to sync variable ${varDef.name} for ${scrape.id}: ${error.message}`);
                }
            }
        }

        if (syncedCount > 0) {
            this.logger.log(`✅ Synced ${syncedCount} workflow variables (${skippedCount} already existed)`);
        }
    }
    ...
    async syncWorkflowSecrets(scrapes: Scrape[]): Promise<void> {
        ...
        for (const scrape of uniqueWorkflows.values()) {
            ...
            for (const varDef of scrape.metadata.variables) {
                ...
                try {
                    const existingSecret = await this.secretsService.getSecretByName(varDef.secretRef);
                    ...
                    syncedCount++;
                    this.logger.debug(`🔐 Created placeholder secret '${varDef.secretRef}' for workflow ${scrape.id}`);
                } catch (error) {
                    this.logger.warn(`⚠️ Failed to sync secret ${varDef.secretRef} for ${scrape.id}: ${error.message}`);
                }
            }
        }

        if (syncedCount > 0) {
            this.logger.log(`🔐 Synced ${syncedCount} secrets (${skippedCount} already existed)`);
        }
    }
     ...
     */
    private async getExistingVariablesMap(workflowId: string): Promise<Map<string, any>> {
    ...
    private async resolveSecretRef(
        ...
        let secret = allSecrets.find(s => s.name.toLowerCase() === secretRef.toLowerCase());
        ...
        if (!secret) {
            ...
            secret = created;
            this.logger.debug(`🔐 Created placeholder secret '${secretRef}' for workflow ${workflowId}`);
        } else {
            this.logger.debug(`🔐 Found existing secret '${secret.name}' for workflow ${workflowId}`);
        }

...
export class ScrapeExecutionService {
    ...
    async executeScrape(
        ...
        try {
            const variablesInfo = await this.prepareVariablesInfo(scrape, variables, previousData);

            // ✨ OBSERVER PATTERN: Publish ScrapeStartedEvent
            await this.eventBus.publish(new ScrapeStartedEvent(runId, scrape.id, runId, variablesInfo));
            ...
            await this.executeSteps(scrape, runId, previousData, storedData, variables);

            // Build and save debug data
            const debugData = this.buildDebugData(previousData);
            await this.databaseService.storeData(scrape.id, '__debugData', JSON.stringify(debugData), runId);

            await this.databaseService.updateRunStatus(runId, 'completed');
            
            // ✨ OBSERVER PATTERN: Publish ScrapeCompletedEvent
            await this.eventBus.publish(new ScrapeCompletedEvent(runId, scrape.id, runId, true));
    ...
    private async executeSteps(
        ...
        for (let stepIndex = 0; stepIndex < scrape.steps.length; stepIndex++) {
            ...
            await this.eventBus.publish(new StepStartedEvent(runId, scrape.id, runId, step.name, stepIndex));
            
            this.scrapeEventsService.updateStepStatus(scrape.id, step.name, stepIndex, 'running', runId);
            const dbStep = await this.databaseService.createStep(runId, step.name, stepIndex);

            try {
                await this.executeActions(scrape, step, stepIndex, dbStep.id, runId, previousData, storedData, variables);

                await this.databaseService.updateStepStatus(dbStep.id, 'completed');
                
                // ✨ OBSERVER PATTERN: Publish StepCompletedEvent
                await this.eventBus.publish(new StepCompletedEvent(runId, scrape.id, runId, step.name, stepIndex));
                
                this.scrapeEventsService.updateStepStatus(scrape.id, step.name, stepIndex, 'completed', runId);
    ...
    private async executeActions(
        ...
        for (let actionIndex = 0; actionIndex < step.actions.length; actionIndex++) {
            ...
            try {
                const result = await this.executeAction(action, scrape, runId, previousData, storedData, variables);
    ...
    private async executeAction(
        ...
        return await this.actionHandlerService.handleAction(
            action, previousData, actionData, storedData, variables
        );
    }
    ...
    private async handleActionError(
        ...
        if (error.message === 'BreakLoop') {
            this.logger.warn(`Exiting scrape because of break on action "${actionName}"`);
            await this.databaseService.updateActionStatus(dbActionId, 'skipped');
            this.scrapeEventsService.updateActionStatus(
                scrape.id, step.name, stepIndex,
                actionName, actionIndex, actionType,
                'skipped', undefined, runId
            );
            return;
        }
        ...
        await this.databaseService.updateActionStatus(dbActionId, 'error', errorMessage);
        
        // ✨ OBSERVER PATTERN: Publish ActionFailedEvent
        await this.eventBus.publish(
            new ActionFailedEvent(runId, scrape.id, runId, actionName, actionType, actionIndex, errorMessage)
        );
        ...
        this.logger.error(`❌ Error executing action "${actionName}": ${errorMessage}`);
        if (errorStack) {
     ...
     */
    private async handleExecutionError(scrape: Scrape, runId: string, error: any): Promise<void> {
    ...
    private checkAbort(scrape: Scrape, runId: string): boolean {
        if (this.puppeteerService.isAborted) {
            ...
            this.scrapeEventsService.scrapeEnded(scrape.id, false, 'Aborted by user', runId);
            return true;
        }
        return false;
    }

...
export class ScrapeDataService {
     ...
     */
    async getStoredDataFromDB(scrapeId: string): Promise<{ storedData: any; isFirstRun: boolean }> {
        try {
            ...
            this.logger.warn(`⚠️ Failed to load stored data from DB: ${error.message}`);
            return { storedData: {}, isFirstRun: true };
        }
    }
     ...
     */
    setScrapeResult(scrapeId: string, stepMap: Map<string, Map<string, any>>): void {

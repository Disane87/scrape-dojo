...
export class SchedulerService implements OnModuleInit, OnModuleDestroy {
        ...
        try {
            ...
            for (const schedule of schedules) {
                if (schedule.cronExpression) {
                    await this.scheduleJob(schedule.scrapeId, schedule.cronExpression, schedule.timezone);
                }
            }
        } catch (error) {
     ...
     */
    async scheduleJob(scrapeId: string, cronExpression: string, timezone: string = 'Europe/Berlin'): Promise<void> {
        ...
        try {
            ...
            if (!nextRun) {
                this.logger.warn(`⚠️ Invalid cron expression for ${scrapeId}: ${cronExpression}`);
                return;
            }
            ...
            this.logger.log(`✅ Scheduled ${scrapeId}: ${cronExpression} (next: ${nextRun.toLocaleString('de-DE')})`);
        } catch (error) {
     ...
     */
    getNextRunTime(cronExpression: string, timezone: string = 'Europe/Berlin'): Date | null {
        try {
    ...
    private async executeScheduledRun(job: ScheduledJob): Promise<void> {
        ...
        try {
            ...
            await this.scrapeService.scrape(job.scrapeId, runId, undefined, 'scheduled');
            ...
            this.logger.error(`❌ Scheduled run failed for ${job.scrapeId}: ${error.message}`);
        }
    }
     ...
     */
    getStatus(): { scrapeId: string; cronExpression: string; nextRun: Date | null }[] {

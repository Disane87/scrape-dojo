import { Controller, Get, Post, Put, Body, Res, HttpStatus, Sse, Param, Query, Delete } from '@nestjs/common';
...
export class ScrapeUIController {
    ...
    async getScrapes() {
        try {
            ...
            const scrapes = await Promise.all(definitions.map(async scrape => {
                const resolvedMetadata = await this.authorResolverService.resolveMetadata(scrape.metadata);

                // Resolve dynamic options for select variables
                if (resolvedMetadata?.variables) {
                    resolvedMetadata.variables = await this.resolveVariableOptions(resolvedMetadata.variables);
                }
        ...
        return Promise.all(variables.map(async (variable) => {
            if (variable.type === 'select' && variable.optionsExpression) {
                try {
                    ...
                    console.error(`Error evaluating optionsExpression for ${variable.name}:`, error);
                }
            }
            return variable;
        }));
    ...
    async getScrapeById(@Param('id') id: string) {
        try {
            ...
            if (scrape.metadata?.variables) {
                scrape.metadata.variables = await this.resolveVariableOptions(scrape.metadata.variables);
            }
    ...
    @Post('run/:scrapeId')
    async runScrape(@Body() body: { runId?: string; variables?: Record<string, any> }, @Res() res: Response) {
        const scrapeId = res.req.params.scrapeId;
        const runId = body?.runId;
        const variables = body?.variables;

        try {
            const result = await this.scrapeService.scrape(scrapeId, runId, variables);
     ...
     */
    @Post('otp/:requestId')
    submitOtp(@Param('requestId') requestId: string, @Body() body: { code: string }, @Res() res: Response) {
        try {
     ...
     */
    @Get('runs')
    async getRuns(@Query('scrapeId') scrapeId?: string, @Query('limit') limit = 50) {
        const runs = scrapeId
        ...
        return runs.map(run => {
            ...
            try {
                if (run.scrapeId) {
                    const scrapeDefinition = this.scrapeService.getScrapeDefinitions().find(s => s.id === run.scrapeId);
                    if (scrapeDefinition && scrapeDefinition.steps) {
                        runDto.steps = this.enrichStepsWithNestedActions(runDto.steps || [], scrapeDefinition.steps);
                    }
                }
            } catch (error) {
                // Falls Scrape-Definition nicht mehr existiert, gebe trotzdem Daten zurück
                console.warn(`Could not enrich run ${run.id}:`, error?.message || error);
            }
    ...
    async getRunById(@Param('runId') runId: string, @Res() res: Response) {
        ...
        try {
            ...
            // Falls Scrape-Definition nicht mehr existiert, gebe trotzdem Daten zurück
            console.warn(`Could not enrich run ${runId} with workflow structure:`, error?.message || error);
        }
    ...
    async getRunDebugData(@Param('runId') runId: string, @Res() res: Response) {
        try {
            ...
            const scrapeDataRepo = this.databaseService.dataSource.getRepository('ScrapeData');
        ...
        try {
            ...
            const extractArtifacts = (obj: any, path: string = '') => {
                ...
                if (obj.type && obj.data !== undefined && 
                    ['auto', 'json', 'table', 'text', 'file', 'image', 'link', 'card'].includes(obj.type)) {
                    ...
                    const contentKey = JSON.stringify({
                        ...
                        dataStr: typeof obj.data === 'string' ? obj.data : JSON.stringify(obj.data)
                    });
                ...
                if (Array.isArray(obj)) {
                    ...
                    const keys = Object.keys(obj).sort((a, b) => {
                        ...
                        const aIsLoop = a.startsWith('loop') && obj[a] && typeof obj[a] === 'object' && 
                            Object.keys(obj[a]).some(k => k.startsWith('iteration_'));
                        const bIsLoop = b.startsWith('loop') && obj[b] && typeof obj[b] === 'object' && 
     ...
     */
    private enrichStepsWithNestedActions(runSteps: any[], configSteps: any[]): any[] {
        ...
        return runSteps.map((runStep, stepIndex) => {
            ...
            return {
                ...runStep,
                actions: this.enrichActionsRecursively(runStep.actions || [], configStep.actions || [])
            };
        });
    }
     ...
     */
    private enrichActionsRecursively(runActions: any[], configActions: any[]): any[] {
        ...
        return runActions.map((runAction, actionIndex) => {
            ...
            if (configAction.action === 'loop' && configAction.params?.actions) {
                ...
                enrichedAction.nestedActions = this.buildNestedActionsTemplate(nestedConfigActions);
                
                // Existierende loopIterations mit childActions anreichern
                if (enrichedAction.loopIterations && Array.isArray(enrichedAction.loopIterations)) {
                    enrichedAction.loopIterations = enrichedAction.loopIterations.map((iteration: any) => ({
                        ...iteration,
                        childActions: iteration.childActions 
                            ? this.enrichActionsRecursively(iteration.childActions, nestedConfigActions)
                            : undefined
                    }));
                }
            }
    ...
    private buildNestedActionsTemplate(configActions: any[]): any[] {
        return configActions.map((configAction: any) => ({
            ...
            nestedActions: configAction.action === 'loop' && configAction.params?.actions
    ...
    async deleteRunsByScrapeId(@Param('scrapeId') scrapeId: string, @Res() res: Response) {
        try {
     ...
     */
    @Post('runs/cleanup')
    async cleanupRuns(@Body() body: { olderThanDays?: number }, @Res() res: Response) {
        try {
    ...
    async getAllScrapeData(@Param('scrapeId') scrapeId: string) {
        const { jobLevel, runs } = await this.databaseService.getAllScrapeData(scrapeId);
        return {
    ...
    async updateSchedule(
        ...
        const schedule = await this.databaseService.updateScheduleConfig(scrapeId, body);

        // Sync scheduler with new settings
        if (body.scheduleEnabled !== undefined || body.cronExpression !== undefined) {

...
export class ScrapeService implements OnModuleInit, OnModuleDestroy {
    ...
    onModuleInit() {
        ...
        this.initializeScrapeDefinitions().catch(err =>
            this.logger.error(`Failed to initialize scrape definitions: ${err.message}`)
        );
     ...
     */
    async scrape(scrapeId: string | null, runId?: string, variables?: Record<string, any>, trigger: RunTrigger = 'manual') {
        ...
        for (const scrape of this.scrapeDefinitions) {
            ...
            try {
                const { storedData, isFirstRun } = await this.dataService.getStoredDataFromDB(scrape.id);
                previousData.set('firstRun', isFirstRun);

                const result = await this.executionService.executeScrape(
                    scrape, currentRunId, previousData, storedData, variables
                );
    ...
    private async initializeScrapeDefinitions(): Promise<void> {
        if (this.initializingScrapeDefinitions) {
            this.logger.warn('⏳ Scrape definitions initialization already running, skipping parallel execution.');
            return;
        }
        ...
        try {
            ...
            this.logger.error(`Failed to initialize scrape definitions: ${error.message}`);
        } finally {
    ...
    private setupFileWatchers(): void {
        try {
            ...
            if (fs.existsSync(sitesPath)) {
                this.sitesWatcher = fs.watch(sitesPath, { recursive: false }, (eventType, filename) => {
                    if (filename && (filename.endsWith('.json') || filename.endsWith('.jsonc'))) {
                        this.logger.log(`📝 Site config changed: ${filename} (${eventType})`);
    ...
    private generateRunId(runId?: string): string {
        return runId || `run-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    }

    /**
     * Bereitet previousData Map mit Variablen vor
     */
    private async preparePreviousData(scrape: Scrape, variables?: Record<string, any>): Promise<Map<string, any>> {
        ...
        if (variables) {
            ...
            this.logger.debug(`📥 Variables loaded: ${Object.keys(variables).join(', ')}`);
        }
        ...
        // Secrets auflösen
        await this.secretsResolverService.resolveSecretsForWorkflow(scrape, previousData);
     ...
     */
    private logScrapeStart(scrape: Scrape, runId: string, variables?: Record<string, any>): void {
        if (variables && Object.keys(variables).length > 0) {
            this.logger.log(`📝 Run-time variables received: ${JSON.stringify(variables)}`);
        }

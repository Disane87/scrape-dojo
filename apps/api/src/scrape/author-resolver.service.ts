...
export class AuthorResolverService {
  ...
  async resolveAuthor(
    ...
    // Direct URL
      ...
      authorString.startsWith('http://') ||
      authorString.startsWith('https://')
    ) {
      author = {
        name: 'Unknown',
        avatar: authorString,
  ...
  async resolveMetadata(
  ...
  ): Promise<ScrapeMetadataResolved | undefined> {
    if (!metadata) {
      return undefined;
    }
    ...
    if (metadata.author) {
      resolved.author = await this.resolveAuthor(
        metadata.author,
        metadata.email,
      );
    }

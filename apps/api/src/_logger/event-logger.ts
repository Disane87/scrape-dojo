...
export class EventLogger extends ConsoleLogger {
  ...
  private static maxLogBytes = Number(
    process.env.SCRAPE_DOJO_LOG_MAX_BYTES ?? 5_000_000,
  );
  ...
  private static cleanupOldLogFiles() {
    try {
      ...
      const fs = require('fs');
      const files = fs
        .readdirSync(EventLogger.logsDir)
      ...
      console.warn(`⚠️ Could not cleanup old log files`);
    }
  }
  ...
  private static writeToFile(
    ...
    try {
      ...
      void fsPromises
        .appendFile(EventLogger.logFilePath, logLine, 'utf-8')
        .catch(() => {
  ...
  private static async trimIfNeeded() {
    try {
      ...
      const stat = await fsPromises
        .stat(EventLogger.logFilePath)
        .catch(() => null);
      if (!stat) return;

      const maxBytes = Number.isFinite(EventLogger.maxLogBytes)
        ? EventLogger.maxLogBytes
        : 5_000_000;
      if (stat.size <= maxBytes) return;

      // Trim anhand der letzten maxLogLines (nur wenn Datei wirklich groß ist)
      const content = await fsPromises.readFile(
        EventLogger.logFilePath,
        'utf-8',
      );
      const lines = content.trim().split('\n');
      if (lines.length > EventLogger.maxLogLines) {
        const newContent =
          lines.slice(-EventLogger.maxLogLines).join('\n') + '\n';
   ...
   */
  static readLogs(limit = 500): Array<{
    timestamp: string;
    level: string;
    context: string;
    message: string;

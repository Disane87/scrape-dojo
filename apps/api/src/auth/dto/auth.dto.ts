...
export class LoginDto {
  ...
  @ApiPropertyOptional({
    description:
      'TOTP code from authenticator app (required when MFA is enabled)',
    example: '123456',
  })
  @IsOptional()
  @IsString()
  ...
  @ApiPropertyOptional({
    description: 'Device fingerprint for device tracking',
  })
  @IsOptional()
  @IsString()
...
export class MfaChallengeResponseDto {
  ...
  @ApiProperty({
    description: 'Short-lived JWT used to perform MFA setup/complete',
  })
  @IsString()
...
export class MfaSetupRequestDto {
  @ApiProperty({
    description: 'MFA challenge token from login/register/OIDC callback',
  })
  @IsString()
  mfaChallengeToken: string;
}
...
export class UserApiKeyListItemDto {
  ...
  @ApiProperty({
    description: 'Non-sensitive prefix shown to identify the key',
    example: 'sdj_12ab34cd56ef',
  })
  keyPrefix: string;

  @ApiPropertyOptional({
    description: 'Last time this key was used (epoch ms)',
  })
  lastUsedAt?: number | null;

  @ApiPropertyOptional({
    description: 'Revoked timestamp (epoch ms), if revoked',
  })
...
export class CreateUserApiKeyResponseDto {
  @ApiProperty({
    description:
      'The plaintext API key. This is only returned once on creation.',
  })
  apiKey: string;

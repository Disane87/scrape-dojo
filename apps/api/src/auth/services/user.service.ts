...
export class UserService {
  ...
  async upsertOidcUser(claims: {
    ...
    this.logger.log(
      `🔍 OIDC upsert: Looking for user with externalId=${claims.sub}, issuer=${claims.iss}`,
    );
    ...
    if (user) {
      this.logger.log(
        `✓ Found existing OIDC user: id=${user.id}, email=${user.email}, mfaEnabled=${user.mfaEnabled}`,
      );
      ...
      this.logger.log(
        `⭐ No existing OIDC user found - checking for local user with email=${claims.email}`,
      );
      ...
      if (existingLocal) {
        this.logger.log(
          `🔗 Linking OIDC to existing local user: id=${existingLocal.id}, email=${existingLocal.email}, currentMfaEnabled=${existingLocal.mfaEnabled}`,
        );
        ...
        if (!Number.isFinite(existingLocal.createdAt))
          existingLocal.createdAt = Date.now();
        if (!Number.isFinite(existingLocal.updatedAt))
          existingLocal.updatedAt = Date.now();

        await this.userRepository.save(existingLocal);
        this.logger.log(
          `✅ Linked OIDC to existing user ${existingLocal.id} - KEEPING existing mfaEnabled=${existingLocal.mfaEnabled}`,
        );
        return existingLocal;
      }

      this.logger.log(
        `⭐ Creating NEW OIDC user for email=${claims.email}, sub=${claims.sub}`,
      );
      ...
      await this.userRepository.save(user);
      this.logger.log(
        `✅ NEW OIDC user created: id=${user.id}, email=${user.email}, mfaEnabled=${user.mfaEnabled || false}`,
      );
    }
  ...
  async changePassword(
    ...
    if (user.provider !== AuthProvider.LOCAL) {
      throw new UnauthorizedException(
        'Password change not available for OIDC accounts',
      );
    }

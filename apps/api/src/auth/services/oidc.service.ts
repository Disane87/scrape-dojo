...
export class OidcService implements OnModuleInit {
    ...
    constructor(private readonly configService: ConfigService) {
        this.oidcConfig = {
            enabled: this.configService.get<string>('SCRAPE_DOJO_AUTH_OIDC_ENABLED', 'false') === 'true',
            issuerUrl: this.configService.get<string>('SCRAPE_DOJO_AUTH_OIDC_ISSUER_URL', ''),
            clientId: this.configService.get<string>('SCRAPE_DOJO_AUTH_OIDC_CLIENT_ID', ''),
            clientSecret: this.configService.get<string>('SCRAPE_DOJO_AUTH_OIDC_CLIENT_SECRET', ''),
            redirectUri: this.configService.get<string>('SCRAPE_DOJO_AUTH_OIDC_REDIRECT_URI', 'http://localhost:3000/auth/oidc/callback'),
            scopes: this.configService.get<string>('SCRAPE_DOJO_AUTH_OIDC_SCOPES', 'openid profile email').split(' '),
            providerName: this.configService.get<string>('SCRAPE_DOJO_AUTH_OIDC_PROVIDER_NAME', 'OIDC Provider'),
        };
    }
    ...
    async onModuleInit() {
        ...
        if (!this.oidcConfig.issuerUrl || !this.oidcConfig.clientId) {
            this.logger.warn('⚠️ OIDC enabled but missing configuration (issuer URL or client ID)');
            this.oidcConfig.enabled = false;
            return;
        }

        try {
            this.logger.log(`🔐 Discovering OIDC provider: ${this.oidcConfig.issuerUrl}`);
            ...
            this.logger.log(`✅ OIDC provider discovered: ${this.oidcConfig.providerName}`);
        } catch (error) {
            this.logger.error(`❌ Failed to discover OIDC provider: ${error.message}`);
            this.oidcConfig.enabled = false;
        }
    }
    ...
    async getAuthorizationUrl(state: string): Promise<string> {
        ...
        const redirectTo = openidClient.buildAuthorizationUrl(this.config, parameters);
        ...
        try {
            ...
            if (!userInfo.email && tokens.access_token) {
                try {
                    ...
                    userInfo.picture = userInfoResponse.picture as string || userInfo.picture;
                } catch (e) {
            ...
            throw new BadRequestException(`OIDC authentication failed: ${error.message}`);
        }
    }
     ...
     * Get logout URL
     */
    getLogoutUrl(idTokenHint?: string, postLogoutRedirectUri?: string): string | null {
        if (!this.config) {
            return null;
        }

...
export class MfaService {
  ...
  isMfaRequired(): boolean {
    const authEnabled =
      this.configService.get<string>('SCRAPE_DOJO_AUTH_ENABLED', 'true') ===
      'true';
    if (!authEnabled) return false;
    return (
      this.configService.get<string>('SCRAPE_DOJO_AUTH_REQUIRE_MFA', 'true') ===
      'true'
    );
  }
  ...
  createChallengeToken(userId: string): string {
    ...
    return this.jwtService.sign(
      {
        sub: userId,
        purpose: 'mfa_challenge' satisfies MfaChallengePayload['purpose'],
      },
  ...
  private getChallengeSecret(): string {
    ...
    const secret = this.configService.get<string>(
      ...
      'mfa-challenge-secret-change-me',
    );
    if (
      nodeEnv === 'production' &&
      secret === 'mfa-challenge-secret-change-me'
    ) {
      throw new Error(

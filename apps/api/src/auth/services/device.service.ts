...
export class DeviceService {
    ...
    async getTrustedDevice(userId: string, deviceFingerprint: string): Promise<TrustedDeviceEntity | null> {
     ...
     */
    isNetworkChangeRisky(previousIp: string | null | undefined, currentIp: string | null | undefined): boolean {
        ...
        if (this.isPrivateOrLoopback(prev, prevVersion) || this.isPrivateOrLoopback(curr, currVersion)) {
            return false;
        }
    ...
    private normalizeIp(value: string | null | undefined): string | null {
        ...
        const unbracketed = raw.startsWith('[') ? raw.replace(/^\[([^\]]+)\].*$/, '$1') : raw;
    ...
    private expandIpv6ToHextets(ip: string): string[] | null {
        ...
        const partsForSplit = hasIpv4Tail ? raw.replace(/(\d+\.\d+\.\d+\.\d+)$/, '0:0') : raw;
        ...
        const hextets = [...left, ...Array(missing).fill('0'), ...right].map((h) => h.padStart(4, '0'));

...
export class AuthService {
  ...
  constructor(
    ...
    this.trustedDeviceRiskIp =
      this.configService.get<string>(
        'SCRAPE_DOJO_AUTH_TRUSTED_DEVICE_RISK_IP',
        'true',
      ) === 'true';
  }
  ...
  async login(
    ...
    const deviceFingerprint =
      ...
      (userAgent && ipAddress
        ? this.deviceService.generateFingerprint(userAgent, ipAddress)
        : null);
    const trustedDevice = deviceFingerprint
      ? await this.deviceService.getTrustedDevice(user.id, deviceFingerprint)
      : null;
    ...
    if (
      ...
      const risky = this.trustedDeviceRiskIp
        ? this.deviceService.isNetworkChangeRisky(
            trustedDevice.lastIpAddress,
            ipAddress,
          )
        : false;
      ...
      if (!risky) {
        ...
        await this.deviceService.updateDeviceLastUsed(
          user.id,
          deviceFingerprint,
          ipAddress,
        );
      ...
      this.logger.log(
        `Trusted device risk signal triggered for user ${user.id}, requiring MFA`,
      );
    }
    ...
    if (finalFingerprint && userAgent && ipAddress) {
      ...
      await this.deviceService.trustDevice(
        user.id,
        finalFingerprint,
        deviceName,
        ipAddress,
  ...
  async handleOidcUser(claims: {
    ...
    this.logger.log(
      `🔐 OIDC User Login: email=${claims.email}, sub=${claims.sub}, issuer=${claims.iss}`,
    );

    const user = await this.userService.upsertOidcUser(claims);

    this.logger.log(
      `✅ User upserted: id=${user.id}, email=${user.email}, mfaEnabled=${user.mfaEnabled}, hasSecret=${!!user.mfaSecret}`,
    );

    const mfaRequired = this.isMfaRequired();
    this.logger.log(
      `🔒 MFA Check: required=${mfaRequired}, userMfaEnabled=${user.mfaEnabled}`,
    );

    if (!mfaRequired) {
      this.logger.log(
        `✨ MFA not required globally - generating tokens directly`,
      );
      return this.generateTokenResponse(user, { mfaVerified: true });
    }
    ...
    this.logger.log(
      `🎫 Creating MFA challenge: setupRequired=${setupRequired}`,
    );
  ...
  async setupMfaFromChallenge(challengeToken: string): Promise<
    { otpauthUrl: string; qrCodeDataUrl: string; secret: string } & {
      userId: string;
    }
  > {
    ...
    await this.withTimeout(
      this.userService.setMfaSecret(
        user.id,
        this.mfaService.encryptSecret(secret),
      ),
      10_000,
      'setMfaSecret',
    );
    ...
    this.logger.debug(
      `[MFA] setup done user=${user.id} in ${Date.now() - startedAt}ms`,
    );
  ...
  private async withTimeout<T>(
    ...
    const timeoutPromise = new Promise<never>((_resolve, reject) => {
      timeoutHandle = setTimeout(
        () =>
          reject(new RequestTimeoutException(`MFA setup timed out (${label})`)),
        ms,
      );
    });
  ...
  async completeMfaFromChallenge(
    ...
    if (finalFingerprint && userAgent && ipAddress) {
      ...
      this.logger.log(
        `Trusted new device for user ${user.id} after MFA completion`,
      );
    }

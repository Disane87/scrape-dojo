...
export class UsersController {
  ...
  @ApiResponse({
    ...
    type: [UserApiKeyListItemDto],
  })
  async listMyApiKeys(
    @CurrentUser() user: UserEntity,
  ): Promise<UserApiKeyListItemDto[]> {
  ...
  @ApiResponse({
    ...
    type: CreateUserApiKeyResponseDto,
  })
  async createMyApiKey(
    @CurrentUser() user: UserEntity,
    @Body() dto: CreateUserApiKeyDto,
  ): Promise<CreateUserApiKeyResponseDto> {
    const { apiKey, entity } = await this.apiKeysService.createForUser(
      user.id,
      dto.name,
    );
    return {
  ...
  async removeAllDevices(
    ...
    const currentFingerprint =
      typeof headerFingerprint === 'string' ? headerFingerprint : null;

...
export class AuthController {
  ...
  @ApiResponse({
    status: 200,
    description: 'MFA required/setup required',
  ...
  async mfaSetup(
    @Body() dto: MfaSetupRequestDto,
  ): Promise<MfaSetupResponseDto> {
    const { otpauthUrl, qrCodeDataUrl, secret } =
      await this.authService.setupMfaFromChallenge(dto.mfaChallengeToken);
  ...
  async mfaComplete(
    ...
    const ipAddress =
      ...
      req.socket.remoteAddress ||
      '';
    return this.authService.completeMfaFromChallenge(
  ...
  async oidcCallback(
    ...
    try {
      ...
      return res.redirect(
        `${this.frontendUrl}/oidc/callback?auth_error=${encodeURIComponent(message)}`,
      );
    }
  }

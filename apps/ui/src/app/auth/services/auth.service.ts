...
export class AuthService {
  ...
  readonly isAuthenticated = computed(
    () => !!this._user() && !!this.getAccessToken(),
  );
  ...
  setup(data: RegisterRequest): Observable<AuthFlowResult> {
    ...
    return this.http
      .post<TokenResponse | MfaChallengeResponse>('/api/auth/setup', data)
      .pipe(
  ...
  login(credentials: LoginRequest): Observable<AuthFlowResult> {
    ...
    const loginData = {
      ...credentials,
      deviceFingerprint:
        credentials.deviceFingerprint || generateDeviceFingerprint(),
    };

    return this.http
      .post<TokenResponse | MfaChallengeResponse>('/api/auth/login', loginData)
      .pipe(
        switchMap((response) => this.handleAuthOrMfaChallenge(response)),
        catchError((err) => this.handleAuthError(err)),
        finalize(() => this._isLoading.set(false)),
  ...
  private isMfaChallengeResponse(
    value: unknown,
  ...
  private handleStoredSessionValidationError(err: unknown): void {
    ...
    console.warn(
      '[AuthService] Stored session validation failed; keeping local session.',
      err,
    );
    this._sessionValidated.set(false);
  }

...
export class ThemeService {
  ...
  readonly theme = computed<Theme>(() =>
    this.guestMode() ? 'system' : this.userTheme(),
  );
  ...
  constructor() {
    ...
    if (
      ...
      if (typeof mediaQuery.addEventListener === 'function') {
        ...
        typeof (mediaQuery as unknown as { addListener?: (cb: any) => void })
          .addListener === 'function'
      ) {
        (
          mediaQuery as unknown as { addListener: (cb: any) => void }
        ).addListener(handler);
      }
    }

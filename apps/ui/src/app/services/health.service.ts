...
export class HealthService {
    ...
    private healthCheck$ = interval(this.healthCheckInterval).pipe(
        startWith(0),
        switchMap(() =>
            this.api.get<HealthStatus>('health').pipe(
                catchError((err) => of({ error: err.message } as unknown as HealthStatus))
            )
        ),
    ...
    private healthSignal = toSignal(this.healthCheck$, {
        initialValue: { health: null as HealthStatus | null, checkedAt: new Date(0) },
    });
    ...
    systemStatus = computed<SystemStatus>(() => {
        ...
        return {
            api: {
                ...
                error: hasError ? (health as unknown as { error: string }).error : undefined,
            },
            sse: {
    ...
    // API Version
    apiVersion = computed(() => this.systemStatus().api.health?.version ?? 'unknown');

    // Docker Status
    isDocker = computed(() => this.systemStatus().api.health?.environment.isDocker ?? false);

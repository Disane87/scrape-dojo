      ...
      <table class="min-w-full divide-y divide-dojo-border">
        <thead class="bg-dojo-surface">
          <tr>
            ...
            <th class="px-4 py-2 text-left text-xs font-medium text-dojo-text-muted uppercase tracking-wider">
              {{ header }}
            </th>
            }
          </tr>
        ...
        <tbody class="divide-y divide-dojo-border">
          ...
          <tr class="hover:bg-dojo-surface transition-colors">
            @for (header of getTableHeaders(); track header) {
            <td class="px-4 py-2 text-sm text-dojo-text whitespace-nowrap">
              @if (isFilePath(row[header])) {
                <div class="flex items-center gap-2">
                  <span class="flex-1">{{ formatCellValue(row[header]) }}</span>
                  <button 
                    ...
                    <iconify-icon icon="lucide:download" class="w-3 h-3"></iconify-icon>
                  </button>
                </div>
              } @else {

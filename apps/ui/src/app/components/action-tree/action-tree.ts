...
export class ActionTreeComponent {
    ...
    isActionExpanded(actionName: string): boolean {
        return this.expandedActions().has(`${this.runId()}-${this.stepName()}-${actionName}-${this.depth()}`);
    }
    ...
    isLoopExpanded(actionName: string): boolean {
        return this.expandedLoops().has(`${this.runId()}-${this.stepName()}-${actionName}-loop-${this.depth()}`);
    }
    ...
    isIterationExpanded(actionName: string, iterationIndex: number): boolean {
        return this.expandedIterations().has(`${this.runId()}-${this.stepName()}-${actionName}-iter-${iterationIndex}-${this.depth()}`);
    }
    ...
    getIterationStatusClass(iteration: LoopIteration): string {
        switch (iteration.status) {
            ...
            default: return 'text-dojo-text-subtle';
        }
    }

    hasChildActions(iteration: LoopIteration): boolean {
        return iteration.childActions !== undefined && iteration.childActions.length > 0;
    }
    ...
    getResultPreview(result: unknown): string {
        ...
        if (result === undefined) return this.transloco.translate('common.preview.undefined');
        ...
        if (Array.isArray(result)) {
            return this.transloco.translate('common.preview.array', { count: result.length });
        }
        if (typeof result === 'object') {
            const keys = Object.keys(result);
            const preview = `${keys.slice(0, 3).join(', ')}${keys.length > 3 ? '...' : ''}`;
            return this.transloco.translate('common.preview.object', { keys: preview });
        }
        return String(result);
    }

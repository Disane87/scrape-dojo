import { Component, computed, input, output, model, CUSTOM_ELEMENTS_SCHEMA } from '@angular/core';
...
export type TabType = 'history' | 'definition' | 'visual' | 'output' | 'variables';

@Component({
    selector: 'app-workflow-header',
    standalone: true,
    imports: [CommonModule, ScrapeIconComponent, ButtonComponent, TranslocoModule],
...
export class WorkflowHeaderComponent {
    ...
    getRunButtonTooltip = computed(() => {
        ...
        if (!triggers.some(t => t.type === 'manual')) return 'Kein manueller Trigger konfiguriert';
    ...
    getScheduleButtonTooltip = computed(() => {
        ...
        const triggers = meta?.triggers;
        if (!triggers || triggers.length === 0) return 'Kein Trigger konfiguriert';
        if (!triggers.some(t => t.type === 'cron')) return 'Kein Cron-Trigger konfiguriert';

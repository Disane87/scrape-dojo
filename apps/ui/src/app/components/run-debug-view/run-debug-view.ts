    ...
    >
      ...
      >
        <div class="flex items-center gap-2">
          ...
          ></iconify-icon>
          <span class="text-sm font-medium text-dojo-text"
            >Debug Data & Artifacts</span
          >
        </div>
        <div class="flex items-center gap-2">
        ...
        @if (!run()) {
          ...
          <div class="p-4 space-y-4">
            ...
            @if (artifacts().length > 0) {
              <div>
                <h3
                  class="text-sm font-semibold text-dojo-text-muted uppercase tracking-wide mb-3"
                >
                  Artifacts
                </h3>
            ...
            <div class="space-y-3">
              <h3
                class="text-sm font-semibold text-dojo-text-muted uppercase tracking-wide"
              >
                Raw Debug Data
              </h3>
              @if (monacoLoaded()) {
                <div
                  id="run-debug-editor"
                  class="min-h-100 border border-dojo-border rounded"
                ></div>
              } @else {
                <pre
                  class="min-h-100 border border-dojo-border rounded bg-dojo-surface p-3 text-xs text-dojo-text overflow-auto whitespace-pre-wrap wrap-break-word"
                  >{{ rawDebugText() }}</pre
                >
              }
            </div>

import { Component, input, computed, signal, inject, CUSTOM_ELEMENTS_SCHEMA } from '@angular/core';
...
export class WorkflowVisualizerComponent {
    ...
    private actionToNode(action: ScrapeAction, parentId: string, index: number): VisualNode {
    ...
    getActionColor(actionType: string | undefined): string {
        if (!actionType) return 'bg-dojo-surface-2 border-dojo-border text-dojo-text-muted';
    ...
    getActionCardClass(actionType: string | undefined, isLoop: boolean, isExpanded: boolean | undefined): string {
    ...
    getImportantParams(params: Record<string, unknown> | undefined): Array<{ key: string, value: string }> {
        if (!params) return [];

        // Priority params to show
        const priorityKeys = ['url', 'selector', 'text', 'message', 'path', 'filename', 'expression', 'elementKey', 'condition'];

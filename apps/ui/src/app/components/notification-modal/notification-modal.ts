...
@Component({
    ...
    template: `
        ...
        <div class="fixed inset-0 z-[60] flex items-end justify-end p-4 pointer-events-none">
            <div class="pointer-events-auto w-full max-w-sm bg-dojo-surface border rounded-lg shadow-xl overflow-hidden animate-slide-in"
                ...
                <div class="flex items-start gap-3 p-4">
                    ...
                    <div class="shrink-0 w-10 h-10 rounded-lg flex items-center justify-center"
                        ...
                        @if (notif.iconUrl && isImageUrl(notif.iconUrl)) {
                            <img [src]="notif.iconUrl" [alt]="notif.title" class="w-6 h-6 rounded object-cover">
                        } @else if (notif.iconUrl) {
                            <app-scrape-icon [icon]="notif.iconUrl" [alt]="notif.title" imgClass="w-6 h-6 rounded" svgClass="w-5 h-5 text-dojo-text" />
                        } @else {
                            <iconify-icon [icon]="'lucide:' + iconName(notif.type)" class="w-5 h-5" [class]="iconClass(notif.type)"></iconify-icon>
                        }
                    </div>

                    <!-- Content -->
                    <div class="flex-1 min-w-0">
                        <h4 class="text-sm font-semibold text-dojo-text truncate">{{ notif.title }}</h4>
                        <p class="text-sm text-dojo-text-muted mt-1">{{ notif.message }}</p>
                ...
                <div class="h-1 bg-dojo-surface-2">
                    <div class="h-full transition-all ease-linear"
                         ...
                         [style.animation]="'shrink ' + notif.autoDismiss + 'ms linear forwards'">
                    </div>
                </div>
                }
            </div>

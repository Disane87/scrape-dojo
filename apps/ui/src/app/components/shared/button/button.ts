...
export class ButtonComponent {
  ...
  buttonClasses = computed(() => {
    const base =
      'inline-flex items-center justify-center font-medium rounded-md transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-dojo-orange/50 focus:ring-offset-1 focus:ring-offset-dojo-bg disabled:opacity-50 disabled:cursor-not-allowed';

    const variants: Record<ButtonVariant, string> = {
      primary:
        'bg-gradient-to-r from-dojo-orange to-amber-600 text-white border border-transparent hover:opacity-90 focus:ring-dojo-orange',
      success:
        'bg-dojo-success-strong hover:bg-dojo-success-strong/90 text-white border border-dojo-success-strong focus:ring-dojo-success-strong',
      secondary:
        'bg-dojo-surface-2 hover:bg-dojo-border text-dojo-text border border-dojo-border focus:ring-dojo-accent',
      danger:
        'bg-dojo-red-strong hover:bg-dojo-danger-strong text-white border border-dojo-red-strong focus:ring-dojo-danger',
      warning:
        'bg-amber-500 hover:bg-amber-600 text-white border border-amber-500 focus:ring-amber-500',
      ghost:
        'bg-transparent hover:bg-dojo-surface-2 text-dojo-text-muted hover:text-dojo-text border border-transparent',
      outline:
        'bg-dojo-surface-2 hover:bg-dojo-border text-dojo-text border border-dojo-border hover:border-dojo-text-muted focus:ring-dojo-accent',
    };

    // Active state overrides
    const activeVariants: Record<ButtonVariant, string> = {
      primary:
        'bg-dojo-orange/20 text-dojo-orange border border-dojo-orange/50',
      success:
        'bg-emerald-500/20 text-emerald-400 border border-emerald-500/50',
      secondary:
        'bg-dojo-surface-2 text-dojo-text border border-dojo-text-muted',
    ...
    const variantClass = this.active()
      ? activeVariants[this.variant()]

                        ...
                        @if (profile()?.provider !== 'local') {
                            ...
                            <div>
                                ...
                                @if (passwordForm.newPassword && passwordForm.newPassword.length < 8) {
                                    <p class="mt-1 text-xs text-red-400">{{ 'settings.security.password_min_length' | transloco }}</p>
                                }
                            </div>
...
export class SettingsModalComponent implements OnInit {
    ...
    getTabClasses(tab: SettingsTab): string {
        const base = 'flex-1 px-4 py-2.5 rounded-lg text-sm font-medium transition-all';
    ...
    removeDevice(deviceId: string): void {
        if (!confirm(this.getTranslation('settings.devices.confirm_remove'))) return;
    ...
    formatDate(timestamp: number): string {
        ...
        if (minutes < 60) return this.getTranslation('settings.devices.minutes_ago', { minutes });
        if (hours === 1) return this.getTranslation('settings.devices.hour_ago');
        if (hours < 24) return this.getTranslation('settings.devices.hours_ago', { hours });

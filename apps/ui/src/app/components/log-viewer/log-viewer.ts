import { Component, input, output, signal, computed, ViewChild, ElementRef, effect, CUSTOM_ELEMENTS_SCHEMA } from '@angular/core';
...
export class LogViewerComponent {
    ...
    selectedLevels = signal<Set<LogLevel>>(new Set(['log', 'error', 'warn', 'debug', 'verbose']));
    ...
    groupedLogs = computed(() => {
        ...
        const groups: Array<{ id: string; context: string; logs: ScrapeEvent[]; hasErrors: boolean; hasWarnings: boolean; startTime: number }> = [];
        
        // Maximaler Zeitabstand in ms für eine Gruppe (5 Sekunden)
        const MAX_TIME_GAP = 5000;
        
        let currentGroup: { id: string; context: string; logs: ScrapeEvent[]; startTime: number } | null = null;
        ...
        for (const log of logs) {
            ...
            const shouldStartNewGroup = !currentGroup || 
                ...
                (timestamp - (currentGroup.logs[currentGroup.logs.length - 1]?.timestamp || 0)) > MAX_TIME_GAP;
    ...
    startResize(event: MouseEvent): void {
        ...
        const onMouseMove = (e: MouseEvent) => {
            ...
            const newHeight = Math.min(Math.max(100, startHeight + delta), window.innerHeight - 100);
    ...
    getLogLevelColor(level?: LogLevel): string {
        switch (level) {
            case 'error': return 'bg-dojo-danger/20 text-dojo-danger';
            case 'warn': return 'bg-dojo-warning/20 text-dojo-warning';
            case 'debug': return 'bg-dojo-text-muted/20 text-dojo-text-muted';
            case 'verbose': return 'bg-dojo-text-subtle/20 text-dojo-text-subtle';
            case 'log':
            default: return 'bg-dojo-accent/20 text-dojo-accent';
        }
    }
    ...
    getLogLevelIconColor(level?: LogLevel): string {
        switch (level) {
            case 'error': return 'text-dojo-danger';
            case 'warn': return 'text-dojo-warning';
            case 'debug': return 'text-dojo-text-muted';
            case 'verbose': return 'text-dojo-text-subtle';
            case 'log':
            default: return 'text-dojo-accent';
        }
    }

    getLevelBadgeClass(level: LogLevel): string {
        const base = 'px-1.5 py-0.5 text-xs rounded cursor-pointer transition-all border';
    ...
    getEventIcon(type: ScrapeEventType): string {
        switch (type) {
            ...
            case 'step-status': return '�';
            case 'action-start': return '⏳';
            case 'action-complete': return '✔️';
            case 'action-status': return '⚡';
            case 'scrape-start': return '🚀';
            case 'scrape-complete': return '🎉';
            case 'scrape-end': return '🏁';
            case 'error': return '❌';
    ...
    formatEventType(type: ScrapeEventType): string {
        switch (type) {
            case 'step-status': return 'step';
            case 'action-status': return 'action';
            case 'loop-iteration': return 'loop';
            case 'scrape-start': return 'start';
            case 'scrape-end': return 'end';
            case 'scrape-complete': return 'complete';
            default: return type;
        }
    }
    ...
    getStatusColor(status: string): string {
        switch (status) {
            case 'running': return 'bg-dojo-accent/20 text-dojo-accent';
            case 'completed': return 'bg-dojo-success/20 text-dojo-success';
            case 'success': return 'bg-dojo-success/20 text-dojo-success';
            case 'error': return 'bg-dojo-danger/20 text-dojo-danger';
            case 'failed': return 'bg-dojo-danger/20 text-dojo-danger';
            case 'pending': return 'bg-dojo-border text-dojo-text-muted';
            case 'skipped': return 'bg-dojo-text-subtle/20 text-dojo-text-subtle';
            default: return 'bg-dojo-border text-dojo-text-muted';
        }
    }
    ...
    getEventTypeColor(type: ScrapeEventType): string {
        switch (type) {
            case 'error': return 'bg-dojo-danger/20 text-dojo-danger';
            case 'step-start': return 'bg-dojo-accent/20 text-dojo-accent';
            case 'step-complete': return 'bg-dojo-success/20 text-dojo-success';
            case 'step-status': return 'bg-dojo-accent/20 text-dojo-accent';
            case 'action-start': return 'bg-dojo-purple/20 text-dojo-purple';
            case 'action-complete': return 'bg-dojo-success/20 text-dojo-success';
            case 'action-status': return 'bg-dojo-purple/20 text-dojo-purple';
            case 'scrape-start': return 'bg-dojo-orange/20 text-dojo-orange';
            case 'scrape-complete': return 'bg-dojo-success/20 text-dojo-success';
            case 'scrape-end': return 'bg-dojo-success/20 text-dojo-success';
            case 'loop-iteration': return 'bg-dojo-purple/20 text-dojo-purple';
            default: return 'bg-dojo-border text-dojo-text-muted';
        }
    }

    getEventBorderColor(event: ScrapeEvent): string {
        if (event.type === 'error' || event.logLevel === 'error') return 'border-dojo-danger bg-dojo-danger/5';
        if (event.logLevel === 'warn') return 'border-dojo-warning bg-dojo-warning/5';
        ...
        if (event.type === 'scrape-complete' || event.type === 'scrape-end') return 'border-dojo-success';

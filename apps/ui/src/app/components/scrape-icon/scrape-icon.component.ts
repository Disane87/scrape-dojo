import { Component, input, signal, computed, effect, inject, CUSTOM_ELEMENTS_SCHEMA } from '@angular/core';
import { CommonModule } from '@angular/common';
import { getIconUrl, isLucideIcon, getLucideIconName } from '../../utils/icon.utils';
...
export class ScrapeIconComponent {
    ...
    constructor() {
        effect(() => {
            const _ = this.icon(); // Track icon changes
            const __ = this.themeService.resolvedTheme(); // Track theme changes
    ...
    currentSrc = computed(() => {
        ...
        const variant = this.themeService.resolvedTheme() === 'dark' ? 'dark' : 'light';

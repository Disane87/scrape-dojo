...
import { Variable, VariableListItem, VariableScope, WorkflowVariable } from '@scrape-dojo/shared';
...
export class VariablesStore extends EntityStore<VariableListItem> {
  ...
  readonly workflowDefinitions = signal<Array<{ workflowId: string; variables: WorkflowVariable[] }>>([]);
  ...
  constructor() {
    super({
      storeName: 'Variables',
      loadFn: async () => {
        const variables = await firstValueFrom(this.http.get<VariableListItem[]>('/api/variables'));
  ...
  private async loadWorkflowDefinitions(): Promise<void> {
    try {
      const definitions = await firstValueFrom(
        this.http.get<Array<{ workflowId: string; variables: WorkflowVariable[] }>>('/api/variables/definitions')
      );
  ...
  getWorkflowDefinitions(workflowId: string): WorkflowVariable[] {
    const def = this.workflowDefinitions().find(d => d.workflowId === workflowId);
  ...
  getByName(name: string, workflowId?: string): VariableListItem | undefined {
    if (workflowId) {
      const workflowVar = this.entities().find(
        v => v.name === name && v.scope === 'workflow' && v.workflowId === workflowId
      );
   ...
   */
  async updateVariable(id: string, updates: { value?: string; description?: string }): Promise<Variable> {
    return firstValueFrom(this.http.put<Variable>(`/api/variables/${id}`, updates));
  }

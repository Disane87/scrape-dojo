        ...
        <div class="bg-[var(--dojo-surface)] rounded-lg border border-[var(--dojo-border)] p-6">
            ...
            @if (isLoadingDevices) {
                <div class="flex items-center justify-center py-12">
                    <iconify-icon icon="mdi:loading" class="text-4xl text-orange-500 animate-spin"></iconify-icon>
                </div>
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="space-y-1 text-sm text-[var(--dojo-text-muted)]">
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span>{{ 'settings.devices.last_used' | transloco }}: {{ formatDate(device.lastUsedAt) }}</span>
                                            </div>
                                            <div class="flex items-center gap-2">
                                                <iconify-icon icon="mdi:calendar-plus"></iconify-icon>
                                                <span>{{ 'settings.devices.added' | transloco }}: {{ formatDate(device.createdAt) }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
export class DeviceSettingsComponent implements OnInit {
    ...
    loadDevices(): void {
        ...
        this.http.get<TrustedDevice[]>('/api/users/me/devices').subscribe({
            ...
            error: (err) => {
                this.errorMessage = err?.error?.message || this.transloco.translate('settings.devices.load_error');
                this.isLoadingDevices = false;
            },
        });
    }
    ...
    removeDevice(deviceId: string): void {
        ...
        this.http.delete(`/api/users/me/devices/${deviceId}`).subscribe({
            next: () => {
                this.successMessage = this.transloco.translate('settings.devices.remove_success');
                this.isLoading = false;
                this.loadDevices();
            },
            error: (err) => {
                this.errorMessage = err?.error?.message || this.transloco.translate('settings.devices.remove_error');
                this.isLoading = false;
            },
        });
    }
    ...
    removeAllDevices(): void {
        if (!confirm(this.transloco.translate('settings.devices.confirm_remove_all'))) {
            return;
        }
        ...
        this.http.delete('/api/users/me/devices').subscribe({
            ...
            error: (err) => {
                this.errorMessage = err?.error?.message || this.transloco.translate('settings.devices.remove_all_error');
                this.isLoading = false;
            },
        });
    }
    ...
    getDeviceIcon(deviceName: string): string {
        ...
        if (name.includes('ios') || name.includes('iphone') || name.includes('ipad')) return 'mdi:apple-ios';
    ...
    formatDate(timestamp: number): string {
        ...
        if (days === 0) {
            ...
            if (hours === 0) {
                ...
                return minutes <= 1 ? this.transloco.translate('settings.devices.just_now') : 
                       this.transloco.translate('settings.devices.minutes_ago', { minutes });
            }
            return hours === 1 ? this.transloco.translate('settings.devices.hour_ago') : 
                   this.transloco.translate('settings.devices.hours_ago', { hours });
        }
        if (days === 1) return this.transloco.translate('settings.devices.yesterday');
        if (days < 7) return this.transloco.translate('settings.devices.days_ago', { days });

        ...
        <div class="bg-dojo-surface rounded-lg border border-dojo-border p-6">
            ...
            <div class="pt-6 border-t border-dojo-border/60">
                <div class="flex items-start justify-between gap-4 mb-4">
                    <div>
                        <h3 class="text-lg font-semibold text-dojo-text mb-1 flex items-center gap-2">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <p class="text-xs text-dojo-text-muted">{{ 'settings.security.api_keys.one_time_notice' | transloco }}</p>
                </div>

                <div class="mt-4">
                    @if (loadingApiKeys) {
                        <div class="flex items-center justify-center py-6">
                            <iconify-icon icon="mdi:loading" class="text-3xl text-orange-500 animate-spin"></iconify-icon>
                        </div>
                        ...
                        <div class="space-y-3">
                            @for (k of apiKeys; track k.id) {
                                <div class="bg-dojo-bg border border-dojo-border rounded-lg p-4 flex items-center justify-between gap-4">
                                    <div class="min-w-0">
...
export class SecuritySettingsComponent implements OnInit {
    ...
    createApiKey(): void {
        ...
        this.http.post<CreateUserApiKeyResponse>('/api/users/me/api-keys', { name }).subscribe({
            next: (res) => {
                ...
                const copyPrompt = this.transloco.translate('settings.security.api_keys.copy_prompt');
    ...
    revokeApiKey(apiKeyId: string): void {
        const ok = confirm(this.transloco.translate('settings.security.api_keys.confirm_revoke'));
        if (!ok) return;
